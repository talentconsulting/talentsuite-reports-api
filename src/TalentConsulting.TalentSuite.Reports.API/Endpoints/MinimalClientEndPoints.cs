...
public class MinimalClientEndPoints
{
    public void RegisterClientEndPoints(WebApplication app)
    {
        ...
        app.MapPut("api/clients/{id}", [Authorize(Policy = "TalentConsultingUser")] async (string id, [FromBody] ClientDto request, CancellationToken cancellationToken, ISender _mediator, ILogger<MinimalProjectEndPoints> logger) =>
        {
            try
            {
